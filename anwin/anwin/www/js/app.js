...
var app = new Framework7({
  ...
  routes: [{
    ...
    },
    {
      ...
      path: '/medewerkerProfiel/',
      url: 'medewerkerProfiel.html',
      options: {
        transition: 'f7-flip',
      },
      on: {
        ...
        pageInit: function (event, page) {
          ...
          $$('#uploadenFoto').on('click', function () {
            ...
            if (document.getElementById("btnImg").files.length == 0) {
              ...
              fotoRef.put(file).then(
                function (snapshot) {}).then(function () {
                var user = firebase.auth().currentUser;
                fotoRef.getDownloadURL().then(function (url) {
                  ...
                  firebase.firestore().collection("medewerker").doc(user.uid).set({
                    ...
                    .then(function () {
                      console.log("Document successfully written!");
                      view.router.navigate('/medewerkerpagina/', {
      ...
      on: {
        pageInit: function (event, page) {
          ...
          $$('#btnHerstelWachtwoord').on('click', function () {
            view.router.navigate('/inloggenmedewerker/', {
              transition: 'f7-circle'
            });
          });
        }
...
function getRegistApi() {
  ...
  $$('#btnVerderFoto').on('click', function () {
    ...
    if (displayNaam.length <= 0) {
      app.dialog.alert('Vul een naam en voornaam in!');
    ...
    firebase.firestore().collection('medewerker').doc(selected_value)
      ...
      .then(function (querySnapshot) {
        querySnapshot.forEach(function (doc) {
          ...
          datum.push({
            from: doc.data().start_datum,
            to: doc.data().eind_datum
          });
        });
...
function loadMedewerkersGegegevens() {
  ...
  if (user != null) {
    user.providerData.forEach(function (profile) {
      ...
      line2 += '<div class="fotoContainer item-media flexible-container"><a href="/wijzigMedewerkerGegevens/"><img style="width: 120px; border: 7px solid #fff; height: 120px; border-radius: 50%" src="' + profile.photoURL + '"></a></div>';
    });

    $$('#medewerkerProfiel').append(line);
    $$('#medewerkerProfiel').append(line2);
  }
}
